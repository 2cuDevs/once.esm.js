...
{
 console.log "$0 version: 2021-10-29 15:46  untested"
}
...
once.scenario.map.init() {
  ...
  SCENARIO_MAP_DI_NAME[2]="postgresql:${SCENARIO_MAP_VERSION[2]}" 
  ...
  SCENARIO_MAP_DI_NAME[5]="postgresql:${SCENARIO_MAP_VERSION[5]}" 
  SCENARIO_MAP_VALUES[5]="/EAM/1_infrastructure/Once/latestServer/PostgreSQL.v${SCENARIO_MAP_VERSION[5]}" 
...
once.dc.status()
{
  ...
  if [ -d "$ONCE_SCENARIO$SELECTED_SCENARIO" ]; then
    echo -e "      current SCENARIO:    folder              \033[1;32m EXISTS \033[0m"
    if [ -f "$ONCE_SCENARIO$SELECTED_SCENARIO/docker-compose.yml" ]; then
      echo -e "      current SCENARIO:    docker-compose.yaml \033[1;32m EXISTS \033[0m"
    else
      echo -e "      current SCENARIO:    docker-compose.yml \033[1;31m MISIING \033[0m"
      tree $ONCE_SCENARIO/$SELECTED_SCENARIO
    fi
  else
    echo -e "      current SCENARIO:    \033[1;31m MISIING \033[0m"
  fi
  echo "
       "
  ...
  local service_details=$(docker ps | grep "$SELECTED_SCENARIO_NAME")

  local image_list=$(docker volume ls | grep "$SELECTED_SCENARIO_NAME-volume" | tr -s ' ' | cut -d\  -f2 | tr '\n' ' ')
  
  local image_status="\033[1;32m EXISTS \033[0m"
  if [ -z "$image_details" ]; then
    image_status="\033[1;31m MISIING \033[0m"
  fi 
  local volume_status="\033[1;32m EXISTS \033[0m"
  if [ -z "$volume_details" ]; then
    volume_status="\033[1;31m MISIING \033[0m"
  fi 
  local network_status="\033[1;32m EXISTS \033[0m"
  if [ -z "$network_details" ]; then
    network_status="\033[1;31m MISIING \033[0m"
  fi
  local service_status="\033[1;32m UP \033[0m"
  if [ -z "$service_details" ]; then
    service_status="\033[1;31m DOWN \033[0m"
  fi    
  ...
  echo "         servoce: $SELECTED_SCENARIO_NAME-service:"
...
function once.dc.create.keycloak()
{
  ...
  if [ -z $(docker network ls --filter name=^${NETWORK_NAME}$ --format="{{ .Name }}") ] ; then 
    ...
    if [ -d "$ONCE_REPO_PREFIX/EAMD.ucp/3rdPartyComponents/org/keycloak/Keycloak/12.0.4/realm-config/" ]; then
      ...
      wget -r https://test.wo-da.de/EAMD.ucp/3rdPartyComponents/org/keycloak/Keycloak/12.0.4/realm-config/
...
function once.dc.create.psql()
{
  ...
  if [ -z $(docker network ls --filter name=^${NETWORK_NAME}$ --format="{{ .Name }}") ] ; then 
...

    {
      ...
      echo "    container_name: $SELECTED_SCENARIO_DC_NAME"
      echo "    image: postgres:12.3"
      echo "    hostname: shifternetzwerk-postgresql"
      echo "    environment:"
      echo "      - POSTGRES_USER=shifternetzwerk"
      echo "      - POSTGRES_PASSWORD=qazwsx123"
      echo "    env_file:"
      ...
      echo "    volumes:"
      echo "      - ./db.init:/docker-entrypoint-initdb.d/"
      echo "      - ./jhipsterSNET.db:/var/lib/postgresql/data"
      echo "    ports:"
      echo "      - 5432:5432"
  ...
  if [ -z $1 ]; then
    ...
    if [ -f "keystore.pkcs12" ]; then
      ...
      # if [ -f "./fullchain.pem" ]; then
        ...
        ln -s ../../Docker/CertBot/1.7.0/config/conf/live/$1/fullchain.pem fullchain.pem
      # fi

      # if [ -f "./private.pem" ]; then
        echo "Coping private.pem file...."
        ln -s ../../Docker/CertBot/1.7.0/config/conf/live/$1/private.pem ./privkey.pem
      # fi
      ...
      # if [ -f "./fullchain.pem" ]; then
        echo "Coping fullchain.pem file...."
        ln -s $ONCE_SCENARIO/EAM/1_infrastructure/Docker/CertBot/1.7.0/config/conf/live/$1/fullchain.pem ./fullchain.pem
      # fi

      # if [ -f "./private.pem" ]; then
        echo "Coping private.pem file...."
        ln -s $ONCE_SCENARIO/EAM/1_infrastructure/Docker/CertBot/1.7.0/config/conf/live/$1/private.pem ./privkey.pem
      # fi
...
function once.update.variables()     # updates the environmental variables: REPO_DIR, COMPONENTS_DIR, SCENARIOS_DIR, ONCE_DIR, ONCE_DEFAULT_SCENARIO, ONCE_LOAD_DIR
{
  ...
  if [ ! -d "$ONCE_SCENARIO" ]; then 
    warn.log "no scenario, using localhost"
    ONCE_SCENARIO=$SCENARIOS_DIR/localhost
  fi
...
function once.startlog(){
  ...
  once.log
  # tail -f $ONCE_REPO_PREFIX/EAMD.ucp/Scenarios/localhost/EAM/1_infrastructure/Once/latestServer/.once
}
...
once.ci() {
  once.completion.install
  $ONCE_REPO_PREFIX/Workspaces/2cuGitHub/once.sh/oosh 
}
